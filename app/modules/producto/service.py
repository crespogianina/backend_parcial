...
class ProductoService:
    ...
    def _validar_reglas_producto_final(
        self,
        es_producto_final: bool,
        ingredientes: list[IngredienteAsignar],
    ) -> None:
        if es_producto_final and ingredientes:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Un producto final no puede tener ingredientes.",
            )
        
        if not es_producto_final and not ingredientes:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail="Un producto con ingredientes debe tener al menos uno.",
            )
    ...
    def _calcular_stock(
        ...
        for ing in ingredientes:
            ...
            unidad_producto = uow.productos.get_unidad_medida(ing.unidad_medida_id)

            if not unidad_producto:
                raise ValueError(f"Unidad de medida {ing.unidad_medida_id} no encontrada")
            ...
            unidades_posibles.append(int(stock_convertido / ing.cantidad))
    ...
    def _stock_maximo_por_ingredientes(self, uow: ProductoUnitOfWork, producto_id: int) -> Optional[int]:
        producto = uow.productos.get_by_id(producto_id)

        if producto and producto.es_producto_final:
            return None
    ...
    def create(self, data: ProductoCreate) -> ProductoPublic:
        ...
        self._validar_reglas_producto_final(data.es_producto_final, data.ingredientes)
        ...
        with ProductoUnitOfWork(self._session) as uow:
            ...
            if data.es_producto_final:
                producto.stock_cantidad = data.stock_cantidad if data.stock_cantidad is not None else 0
            else:
                producto.stock_cantidad = self._calcular_stock(uow, data.ingredientes, False)
    ...
    def update(self, producto_id: int, data: ProductoUpdate) -> ProductoPublic:
        with ProductoUnitOfWork(self._session) as uow:
            ...
            if data.es_producto_final is not None or data.ingredientes is not None:
                es_final_nuevo = (
                    data.es_producto_final
                    if data.es_producto_final is not None
                    else producto.es_producto_final
                )
                ingredientes_entrantes = data.ingredientes if data.ingredientes is not None else []
                self._validar_reglas_producto_final(es_final_nuevo, ingredientes_entrantes)

            patch = data.model_dump(
                exclude_unset=True,
                exclude={"categorias", "ingredientes", "stock_cantidad"},
            )
            ...
            if data.ingredientes is not None:
                ...
                if not producto.es_producto_final:
                    producto.stock_cantidad = self._calcular_stock(
                        uow, data.ingredientes, False
                    )
    ...
    def asociar_ingrediente(self, producto_id: int, data: IngredienteAsignar) -> ProductoPublic:
        with ProductoUnitOfWork(self._session) as uow:
            ...
            if producto.es_producto_final:
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail="No se pueden asociar ingredientes a un producto final.",
                )
