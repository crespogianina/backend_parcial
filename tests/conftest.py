...
import os
os.environ["POSTGRES_DB"] = "test_food_store"
os.environ["DATABASE_URL"] = (
    f"postgresql://{os.environ['POSTGRES_USER']}:{os.environ['POSTGRES_PASSWORD']}"
    f"@{os.environ['POSTGRES_HOST']}:{os.environ['POSTGRES_PORT']}/test_food_store"
)
...
@pytest.fixture(name="seed_roles", autouse=True)
...
def get_auth_headers(session: Session, user: Usuario) -> dict[str, str]:
    roles = session.exec(
        select(UsuarioRol).where(UsuarioRol.usuario_id == user.id)
    ).all()
    roles_codigos = [r.rol_codigo for r in roles]
    token = create_access_token(
        data={"sub": user.username, "roles": roles_codigos}
    )
    return {"access_token": token}


def set_auth_cookie(client: TestClient, session: Session, user: Usuario) -> None:
    headers = get_auth_headers(session, user)
    client.cookies.set("access_token", headers["access_token"])
...
def stock_user_fixture(session: Session):
    user = Usuario(
        username="test_stock",
        nombre="Test",
        apellido="Stock",
...
def unidad_ud_fixture(session: Session, seed_unidades_medida):
    return session.exec(select(UnidadMedida).where(UnidadMedida.simbolo == "ud")).first()


@pytest.fixture(name="unidad_g")
def unidad_g_fixture(session: Session, seed_unidades_medida):
    return session.exec(select(UnidadMedida).where(UnidadMedida.simbolo == "g")).first()
...
def pedido_pendiente_fixture(
    session: Session, client_user, producto_final,
    seed_estados_pedido, seed_formas_pago
):
    pedido = Pedido(
